...
export class ExpensesService {
  ...
  async findAll(userId: string): Promise<Expense[]> {
    return this.expenseModel.find({ userId: new Types.ObjectId(userId) }).exec();
  }
  ...
  async findOne(userId: string, id: string): Promise<Expense> {
    ...
    if (!expense.userId.equals(userId)) throw new ForbiddenException('Unauthorized');
    return expense;
  }

  async update(userId: string, id: string, expenseData: Partial<CreateExpenseDto>): Promise<Expense> {
    const expense = await this.expenseModel.findById(id).exec();
    if (!expense) throw new NotFoundException(`Expense with ID ${id} not found`);
    if (!expense.userId.equals(userId)) throw new ForbiddenException('Unauthorized');
